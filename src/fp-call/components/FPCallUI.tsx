...
export const FPCallUI = ({
  ...
  const remotePlaybackVolume = speakerOn ? 100 : 0;
  const hiddenRemoteAudioStyle: React.CSSProperties = {
    position: "absolute",
    width: 1,
    height: 1,
    margin: -1,
              ...
              <>
                {remoteUsers.map((user) => (
                  ...
                  >
                    <RemoteUser
                      user={user}
                      playVideo={!!(user.hasVideo && user.videoTrack)}
                      playAudio={speakerOn}
                      volume={remotePlaybackVolume}
                      style={{
                        width: "100%",
